...
def parse_args():
    parser = argparse.ArgumentParser(description="Visualize latent hierarchy with MNIST image overlays per stage.")
    parser.add_argument("--tree-json", type=str, required=True, help="Path to JSON from cluster_mnist_latent_tree.py")
    ...
    parser.add_argument("--output", type=str, default="latents/mnist_tree_images.png", help="Output PNG path")
...
def _draw_image_grid(
    ...
    for i, idx in enumerate(pick):
        ...
        arr = images[idx]
        im = Image.fromarray(arr.astype(np.uint8), mode="L").resize((tile, tile), resample=Image.Resampling.NEAREST)
        draw_img.paste(im.convert("RGB"), (xi, yi))
...
def main():
    ...
    width = row_header_w + max_k * (cell_w + gap_x) + 20
    height = title_h + num_steps * (cell_h + gap_y) + 20

    canvas = Image.new("RGB", (width, height), color=(250, 250, 250))
    draw = ImageDraw.Draw(canvas)

    draw.text((12, 12), "MNIST Latent Hierarchy by Recursion Step", fill=(20, 20, 20))
    draw.text((12, 30), f"Sorted by parent links; edges filtered by overlap>={args.min_edge_overlap}, purity>={args.min_edge_purity}", fill=(70, 70, 70))
        ...
        for c in step_orders[step_idx]:
            ...
            if step < num_steps:
                ...
                if n > 0:
                    ...
                    rep = Image.fromarray(images[rep_idx].astype(np.uint8), mode="L").resize((84, 84), resample=Image.Resampling.NEAREST)
                    canvas.paste(rep.convert("RGB"), (x0 + 18, y0 + 30))
                    draw.text((x0 + 4, y0 + 118), f"class~{maj}", fill=(20, 20, 20))
                else:
                    draw.text((x0 + 30, y0 + 60), "empty", fill=(120, 120, 120))
