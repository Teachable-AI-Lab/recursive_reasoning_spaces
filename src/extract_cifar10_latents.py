...
from evaluate_cifar10 import load_config
from pretrain import create_dataloader, create_model, get_default_device


def parse_args():
    parser = argparse.ArgumentParser(description="Extract intermediate CIFAR-10 latents across recursion steps.")
    parser.add_argument("--checkpoint", type=str, required=True, help="Path to checkpoint file")
    parser.add_argument("--config-dir", type=str, default="config", help="Hydra config directory")
    parser.add_argument("--config-name", type=str, default="cfg_pretrain_cifar10", help="Hydra config name fallback")
    ...
    parser.add_argument("--output", type=str, default="latents/cifar10_step_latents.npz", help="Output .npz path")
...
def main():
    ...
    with torch.inference_mode():
        for _set_name, batch, _global_batch_size in loader:
            ...
            images.append((inp - 1).reshape(take, 32, 32, 3).detach().cpu().numpy().astype(np.uint8))
