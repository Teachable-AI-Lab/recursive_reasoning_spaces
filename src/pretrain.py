...
try:
    ...
    print("adam-atan2 backend unavailable; falling back to torch.optim.AdamW")
...
def load_checkpoint(model: nn.Module, config: PretrainConfig, device: torch.device):
    if config.load_checkpoint is not None:
        ...
        # Resize/reset puzzle emb if needed, or drop it when current model does not use puzzle embeddings
        model_has_puzzle_emb = hasattr(model.model, "puzzle_emb") and hasattr(model.model.puzzle_emb, "weights")  # type: ignore
        puzzle_emb_keys = [k for k in state_dict.keys() if ".puzzle_emb." in k]

        if model_has_puzzle_emb:
            expected_shape: torch.Size = model.model.puzzle_emb.weights.shape  # type: ignore
            for key in puzzle_emb_keys:
                puzzle_emb = state_dict[key]
                if puzzle_emb.shape != expected_shape:
                    print(f"Resetting puzzle embedding as shape is different. Found {puzzle_emb.shape}, Expected {expected_shape}")
                    state_dict[key] = torch.mean(puzzle_emb, dim=0, keepdim=True).expand(expected_shape).contiguous()
        else:
            for key in puzzle_emb_keys:
                state_dict.pop(key)
        ...
        if missing_keys:
            print(f"Checkpoint load missing keys ({len(missing_keys)}): {missing_keys[:5]}{' ...' if len(missing_keys) > 5 else ''}")
        if unexpected_keys:
            print(f"Checkpoint load unexpected keys ({len(unexpected_keys)}): {unexpected_keys[:5]}{' ...' if len(unexpected_keys) > 5 else ''}")
...
def evaluate(
    ...
    with torch.inference_mode():
        ...
        for set_name, batch, global_batch_size in eval_loader:
            processed_batches += 1
            should_log_batch = (processed_batches == 1) or (processed_batches % eval_log_interval == 0)
            if rank == 0 and should_log_batch:
...
def launch(hydra_config: DictConfig):
    ...
    if "LOCAL_RANK" in os.environ:
        if not torch.cuda.is_available():
            raise RuntimeError("Distributed training via torchrun requires CUDA/NCCL in this script.")
